                     ...
                     <Grid xs={12} md={6}>
                        ...
                        {item.itemSellStatus === 'SOLD_OUT' ? (
                           ...
                           <Box sx={{ display: 'flex', flexDirection: 'column', gap: '16px', width: '300px' }}>
                              <NumberInput aria-label="Demo number input" placeholder="수량" value={} onChange={} min={1} max={item.stockNumber} />
                  ...
                  <Grid container spacing={2}>
                     {item.Imgs.map((img, index) => (
                        <Grid xs={12} sm={6} md={4} key={index}>
                           <CardMedia component="img" image={} alt={`상세 이미지 ${index + 1}`} sx={{ width: '100%', borderRadius: '8px' }} />
                        </Grid>
                     ))}
                  </Grid>
               </Box>
